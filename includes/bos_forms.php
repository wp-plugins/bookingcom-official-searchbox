...
function bos_searchbox_settings_fields_array() {
    ...
    $fields[ 'aid' ] =  array( 'aid', 'text', __( 'Your affiliate ID' , BOS_TEXT_DOMAIN ) , __( 'Your affiliate ID is a unique number that allows Booking.com to track commission. If you are not an affiliate yet, <a href="http://www.booking.com/content/affiliates.html" target="_blank">check our affiliate programme</a> and get an affiliate ID. It\'s easy and fast. Start earning money, <a href="https://secure.booking.com/partnerreg.html" target="_blank">sign up now!</a>', BOS_TEXT_DOMAIN ), 6, 10, 0, 'main', __( 'e.g.' , BOS_TEXT_DOMAIN ) . ' ' . BOS_DEFAULT_AID ) ;        
    ...
    $fields[ 'buttonpos' ] = array( 'buttonpos', 'select', __( 'Button position' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'main','' ) ;
    $fields[ 'logopos' ] = array( 'logopos', 'select', __( 'Logo position' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'main','' ) ;
    
    
    //$fields[ 'save_button_on_widget' ] = array( 'save_button_on_widget', 'checkbox', __( 'Enable this if you would like to have a save button on widget ( useful for plugins like "Fixed Widget" )' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'main' ) ;    
    ...
    $fields[ 'destination' ] = array( 'destination', 'text', __( 'Destination', BOS_TEXT_DOMAIN ) , __( 'You can pre-fill this field with a specific destination ( e.g. Amsterdam )', BOS_TEXT_DOMAIN ), '', 18, 0, 'destination',  __( 'e.g. Amsterdam' , BOS_TEXT_DOMAIN ) ) ;
    $fields[ 'dest_type' ] = array( 'dest_type', 'select', __( 'Destination type', BOS_TEXT_DOMAIN ) , '', '', '', 0, 'destination', '' ) ;
    $fields[ 'dest_id' ] = array( 'dest_id', 'text', __( 'Destination ID ( e.g. -2140479 for Amsterdam )', BOS_TEXT_DOMAIN ) , '<a href="#" id="bos_info_displayer" title="Info box"><img  style="border: none;" src="' . BOS_IMG_PLUGIN_DIR . '/bos_info_icon.png" alt="info"></a>', '', 25, 0, 'destination',  __( 'e.g. -2140479 for Amsterdam' , BOS_TEXT_DOMAIN ) ) ;
    $fields[ 'display_in_custom_post_types' ] = array( 'display_in_custom_post_types', 'text', __( 'Enable meta boxes for these Custom Post Type ( use the slug )', BOS_TEXT_DOMAIN ) , __( 'If multiple, use "," ( comma ) i.e.: cpt1, cpt2, ctp3', BOS_TEXT_DOMAIN ), '', '', 0, 'destination', '' ) ;
    ...
    $fields[ 'maintitle' ] = array( 'maintitle','text', __( 'Default title ( e.g. Search hotels and more... )' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'wording', __( 'Search hotels and more...', BOS_TEXT_DOMAIN ) ) ;
    $fields[ 'dest_title' ] = array( 'dest_title','text', __( 'Destination ( e.g. Destination )' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'wording', __( 'Destination', BOS_TEXT_DOMAIN ) ) ;
    $fields[ 'checkin' ] = array( 'checkin','text', __( 'Check-in ( e.g. Check-in date )' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'wording', __( 'Check-in date', BOS_TEXT_DOMAIN ) ) ;
    $fields[ 'checkout' ] = array( 'checkout','text', __( 'Check-out ( e.g. Check-out date )' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'wording', __( 'Check-out date', BOS_TEXT_DOMAIN ) ) ;
    $fields[ 'submit' ] = array( 'submit','text', __( 'Submit button ( e.g. Search )' , BOS_TEXT_DOMAIN ) , '', '', '', 0, 'wording', __( 'Search', BOS_TEXT_DOMAIN ) ) ;
    ...
    foreach( $arrayFields as $field ) {
        ...
        add_settings_field(            
            ...
            'bos_searchbox_'.$field[ 7 ],//section
             $args = array( $field[ 0 ], $field[ 1 ], $field[ 3 ], $field[ 4 ], $field[ 5 ], $field[ 8 ] ) //args
...
function bos_searchbox_section_destination() {
    ...
    echo '<p><em>'. __( 'Use the following fields to select a specific destination. <em>Destination types</em> and <em>IDs</em> make guest searches more accurate.', BOS_TEXT_DOMAIN ) .'</em><span></span></p>'; 
    echo '</div>';    
}
...
function bos_searchbox_section_color() {
    ...
    echo '<p><em>' . __( 'Enter your colour scheme settings here.', BOS_TEXT_DOMAIN ) .'</em><span></span></p>';
    echo '</div>';
}
...
function bos_searchbox_section_wording() {
    ...
    echo '<p><em>' . __( 'Customise the search box text here.', BOS_TEXT_DOMAIN ) .'</em><span></span></p>'; 
    echo '</div>';
}
...
function bos_searchbox_settings_field( $args ) {
        ...
        if ( $args[ 1 ] == 'text' ) {            
            ...
            if ( !empty( $args[ 5 ] ) ) { $output .= ' placeholder="' . $args[ 5 ] . '" '; }
        ...
        elseif ( $args[ 1 ] == 'select' ) {
            ...
            if ( $args[ 0 ] == 'dest_type' ) {
                ...
                $output .= '<option value="airport" ' . selected( 'airport', $fields_value, false ) . ' >' . __( 'airport' , BOS_TEXT_DOMAIN) . '</option>' ;
            ...
            if( $field[ 0 ] == 'aid' ) {                
                ...
                // Check if user is placing correct affiliate ID and not partner ID
                    ...
                    if( $input[ $field[ 0 ] ][ 0 ] == '4' ) { // check first number of the converted value into a string 
                        ...
                        $error = true ;                    
                        $message[] = '"' . $field[ 2 ] . '": '. __( 'affiliate ID is different from partner ID: should start with a 3 or an 8. Please change it.', BOS_TEXT_DOMAIN ) . '<br>' ;
            ...
            if ( $field[ 0 ] == 'display_in_custom_post_types' ) {             
                ...
                if( !empty( $input[ $field[ 0 ] ] )  ) {
                    ...
                    if( preg_match('/^[a-zA-Z0-9-_,]+$/', $input[ $field[ 0 ] ]  ) ) {
                        ...
                        $error = true ;                    
                        $message[] = '"' .$field[ 2 ] . '": '. __( 'Use just alfanumeric strings and commas for multiple slugs', BOS_TEXT_DOMAIN ) . '<br>' ;
