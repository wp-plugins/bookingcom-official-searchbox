...
function bos_searchbox_add_page() {
    ...
    add_options_page( 
        'Booking.com Search Box settings', // Page title on browser bar 
...
function bos_create_searchbox( $searchbox_options, $preview ) {
    ...
    $maintitle = !empty( $options[ 'maintitle' ] ) ? $options[ 'maintitle' ] : __( 'Search hotels and more...', BOS_TEXT_DOMAIN ) ;
    $dest_title = !empty( $options[ 'dest_title' ] ) ? $options[ 'dest_title' ] : __( 'Destination', BOS_TEXT_DOMAIN ) ;
