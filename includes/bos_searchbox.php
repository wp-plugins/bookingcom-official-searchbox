...
<div id="flexi_searchbox" style="<?php echo  $bgcolor ? 'background-color:'. $bgcolor .';' : ''; echo  $textcolor ? 'color:'. $textcolor .';' : '' ; echo $widget_width ? 'width:'. $widget_width . 'px;' : '' ;  ?>" >
    ...
    <div id="b_searchboxInc">
        <h3 class="search-box-title-1"  style="<?php echo  $textcolor ? 'color:' . $textcolor . ';' : 'color: #003580;' ; ?>"><?php echo $maintitle ? $maintitle : __( 'Need a hotel?' , BOS_TEXT_DOMAIN ) ; ?></h3>
         <form id="b_frm" action="<?php echo $target_page ; ?>" method="get" target="_blank" onsubmit="return sp.validation.validSearch();">
            ...
            <div id="b_frmInner">
                ...
                <input type="hidden" name="error_url" value="<?php echo $target_page ; ?>?aid=<?php echo !empty( $options[ 'aid' ] ) ? $options[ 'aid' ] : BOS_DEFAULT_AID ; ?>;" />
                <input type="hidden" name="si" value="ai,co,ci,re,di" />
                <input type="hidden" name="label" value="wp-searchbox-widget-<?php echo !empty( $options[ 'aid' ] ) ? $options[ 'aid' ] : BOS_DEFAULT_AID ; ?>" />                   
                <input type="hidden" name="aid" value="<?php echo !empty( $options[ 'aid' ] ) ? $options[ 'aid' ] : BOS_DEFAULT_AID ; ?>" />
                <input type="hidden" name="utm_campaign" value="search_box" /> 
                <input type="hidden" name="utm_medium" value="sp" /> 
                <input type="hidden" name="utm_term" value="wp-widget-<?php echo !empty( $options[ 'aid' ] ) ? $options[ 'aid' ] : BOS_DEFAULT_AID ; ?>" /> 
                ...
                <div class="b_submitButton_wrapper">
                    <input class="b_submitButton" type="submit" value="<?php  echo $submit ? $submit : __( 'search' , BOS_TEXT_DOMAIN ) ; ?>" style="<?php echo  $submit_bgcolor ? 'background-color:'. $submit_bgcolor . ';' : '' ; echo  $submit_textcolor ? 'color:'. $submit_textcolor . ';' : '' ; echo $submit_bordercolor ? 'border-color:' . $submit_bordercolor . ';' : '' ;  ?>"/>
                </div><!-- .b_submitButton_wrapper-->
