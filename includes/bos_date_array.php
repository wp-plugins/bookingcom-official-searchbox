<?php 
    ...
    function bos_dateSelector( $month_format, $calendar, $checkin, $checkout, $textcolor )  { 
        ...
        $monthName = array( 1 => __( 'Jan', BOS_TEXT_DOMAIN ), __( 'Feb', BOS_TEXT_DOMAIN ), __( 'Mar', BOS_TEXT_DOMAIN ), 
            __( 'Apr', BOS_TEXT_DOMAIN ), __( 'May', BOS_TEXT_DOMAIN ), __( 'Jun', BOS_TEXT_DOMAIN ), __( 'Jul', BOS_TEXT_DOMAIN ), __( 'Aug', BOS_TEXT_DOMAIN ), 
        ...
        for( $currentMonth = intval( date( "m", $currentDate) ); $currentMonth <= 12; $currentMonth++ ) {
        ...
        for( $currentMonth = 1 ; $currentMonth <= ( intval( date( "m", $currentDate ) ) - 1) ; $currentMonth++ ) {
        ...
        for( $tomorrowMonth = intval( date( "m", $tomorrow ) ); $tomorrowMonth <= 12; $tomorrowMonth++ ) {
            
            if( intval( date( "m", $tomorrow ) ) == $tomorrowMonth ) { $selected = 'selected="selected"' ; } else { $selected = '' ; }
            $output .= "<option " . $selected . " value='" . $tomorrowYear . "-" . $tomorrowMonth . "'>" . $monthName[ $tomorrowMonth ] . "&nbsp" . $tomorrowYear . "</option>\n" ;
                       
        }
        
        for( $tomorrowMonth = 1 ; $tomorrowMonth <= (intval( date( "m", $tomorrow) ) - 2) ; $tomorrowMonth++ ) {
            
            $output .= "<option value='" . $nextYear . "-" . $tomorrowMonth . "'>" . $monthName[ $tomorrowMonth ] . "&nbsp". $nextYear . "</option>\n" ;
